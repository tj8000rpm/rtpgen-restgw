# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: rtpgen/ipc_pack.proto

import sys
_b=sys.version_info[0]<3 and (lambda x:x) or (lambda x:x.encode('latin1'))
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
from google.protobuf import descriptor_pb2
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()




DESCRIPTOR = _descriptor.FileDescriptor(
  name='rtpgen/ipc_pack.proto',
  package='',
  syntax='proto2',
  serialized_pb=_b('\n\x15rtpgen/ipc_pack.proto\"\xb3\x01\n\x0bRtpConfigV1\x12\x0f\n\x07\x65nabled\x18\x01 \x01(\x08\x12\x13\n\x0bip_dst_addr\x18\x03 \x01(\r\x12\x13\n\x0bip_src_addr\x18\x04 \x01(\r\x12\x14\n\x0cudp_dst_port\x18\x05 \x01(\r\x12\x14\n\x0cudp_src_port\x18\x06 \x01(\r\x12\x15\n\rrtp_timestamp\x18\x07 \x01(\r\x12\x14\n\x0crtp_sequence\x18\x08 \x01(\r\x12\x10\n\x08rtp_ssrc\x18\t \x01(\r\"\xab\x03\n\x0eRtpgenIPCmsgV1\x12-\n\x0crequest_code\x18\x01 \x01(\x0e\x32\x17.RtpgenIPCmsgV1.Request\x12/\n\rresponse_code\x18\x02 \x01(\x0e\x32\x18.RtpgenIPCmsgV1.Response\x12\x0e\n\x06portid\x18\x03 \x01(\r\x12\x13\n\x0bid_selector\x18\x04 \x01(\x05\x12\x0c\n\x04size\x18\x05 \x01(\x05\x12 \n\nrtp_config\x18\x06 \x01(\x0b\x32\x0c.RtpConfigV1\"7\n\x07Request\x12\n\n\x06\x43REATE\x10\x00\x12\x08\n\x04READ\x10\x01\x12\n\n\x06UPDATE\x10\x02\x12\n\n\x06\x44\x45LETE\x10\x03\"\xaa\x01\n\x08Response\x12\x0c\n\x07SUCCESS\x10\xc8\x01\x12\x16\n\x11\x45RROR_BAD_REQUEST\x10\x90\x03\x12\x14\n\x0f\x45RROR_FORBIDDEN\x10\x93\x03\x12\x14\n\x0f\x45RROR_NOT_FOUND\x10\x94\x03\x12\x13\n\x0e\x45RROR_CONFLICT\x10\x99\x03\x12\x17\n\x12\x45RROR_SERVER_ERROR\x10\xf4\x03\x12\x1e\n\x19\x45RROR_SERVICE_UNAVAILABLE\x10\xf7\x03')
)
_sym_db.RegisterFileDescriptor(DESCRIPTOR)



_RTPGENIPCMSGV1_REQUEST = _descriptor.EnumDescriptor(
  name='Request',
  full_name='RtpgenIPCmsgV1.Request',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='CREATE', index=0, number=0,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='READ', index=1, number=1,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='UPDATE', index=2, number=2,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DELETE', index=3, number=3,
      options=None,
      type=None),
  ],
  containing_type=None,
  options=None,
  serialized_start=407,
  serialized_end=462,
)
_sym_db.RegisterEnumDescriptor(_RTPGENIPCMSGV1_REQUEST)

_RTPGENIPCMSGV1_RESPONSE = _descriptor.EnumDescriptor(
  name='Response',
  full_name='RtpgenIPCmsgV1.Response',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='SUCCESS', index=0, number=200,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ERROR_BAD_REQUEST', index=1, number=400,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ERROR_FORBIDDEN', index=2, number=403,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ERROR_NOT_FOUND', index=3, number=404,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ERROR_CONFLICT', index=4, number=409,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ERROR_SERVER_ERROR', index=5, number=500,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ERROR_SERVICE_UNAVAILABLE', index=6, number=503,
      options=None,
      type=None),
  ],
  containing_type=None,
  options=None,
  serialized_start=465,
  serialized_end=635,
)
_sym_db.RegisterEnumDescriptor(_RTPGENIPCMSGV1_RESPONSE)


_RTPCONFIGV1 = _descriptor.Descriptor(
  name='RtpConfigV1',
  full_name='RtpConfigV1',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='enabled', full_name='RtpConfigV1.enabled', index=0,
      number=1, type=8, cpp_type=7, label=1,
      has_default_value=False, default_value=False,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='ip_dst_addr', full_name='RtpConfigV1.ip_dst_addr', index=1,
      number=3, type=13, cpp_type=3, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='ip_src_addr', full_name='RtpConfigV1.ip_src_addr', index=2,
      number=4, type=13, cpp_type=3, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='udp_dst_port', full_name='RtpConfigV1.udp_dst_port', index=3,
      number=5, type=13, cpp_type=3, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='udp_src_port', full_name='RtpConfigV1.udp_src_port', index=4,
      number=6, type=13, cpp_type=3, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='rtp_timestamp', full_name='RtpConfigV1.rtp_timestamp', index=5,
      number=7, type=13, cpp_type=3, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='rtp_sequence', full_name='RtpConfigV1.rtp_sequence', index=6,
      number=8, type=13, cpp_type=3, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='rtp_ssrc', full_name='RtpConfigV1.rtp_ssrc', index=7,
      number=9, type=13, cpp_type=3, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  syntax='proto2',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=26,
  serialized_end=205,
)


_RTPGENIPCMSGV1 = _descriptor.Descriptor(
  name='RtpgenIPCmsgV1',
  full_name='RtpgenIPCmsgV1',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='request_code', full_name='RtpgenIPCmsgV1.request_code', index=0,
      number=1, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='response_code', full_name='RtpgenIPCmsgV1.response_code', index=1,
      number=2, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=200,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='portid', full_name='RtpgenIPCmsgV1.portid', index=2,
      number=3, type=13, cpp_type=3, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='id_selector', full_name='RtpgenIPCmsgV1.id_selector', index=3,
      number=4, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='size', full_name='RtpgenIPCmsgV1.size', index=4,
      number=5, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='rtp_config', full_name='RtpgenIPCmsgV1.rtp_config', index=5,
      number=6, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
    _RTPGENIPCMSGV1_REQUEST,
    _RTPGENIPCMSGV1_RESPONSE,
  ],
  options=None,
  is_extendable=False,
  syntax='proto2',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=208,
  serialized_end=635,
)

_RTPGENIPCMSGV1.fields_by_name['request_code'].enum_type = _RTPGENIPCMSGV1_REQUEST
_RTPGENIPCMSGV1.fields_by_name['response_code'].enum_type = _RTPGENIPCMSGV1_RESPONSE
_RTPGENIPCMSGV1.fields_by_name['rtp_config'].message_type = _RTPCONFIGV1
_RTPGENIPCMSGV1_REQUEST.containing_type = _RTPGENIPCMSGV1
_RTPGENIPCMSGV1_RESPONSE.containing_type = _RTPGENIPCMSGV1
DESCRIPTOR.message_types_by_name['RtpConfigV1'] = _RTPCONFIGV1
DESCRIPTOR.message_types_by_name['RtpgenIPCmsgV1'] = _RTPGENIPCMSGV1

RtpConfigV1 = _reflection.GeneratedProtocolMessageType('RtpConfigV1', (_message.Message,), dict(
  DESCRIPTOR = _RTPCONFIGV1,
  __module__ = 'rtpgen.ipc_pack_pb2'
  # @@protoc_insertion_point(class_scope:RtpConfigV1)
  ))
_sym_db.RegisterMessage(RtpConfigV1)

RtpgenIPCmsgV1 = _reflection.GeneratedProtocolMessageType('RtpgenIPCmsgV1', (_message.Message,), dict(
  DESCRIPTOR = _RTPGENIPCMSGV1,
  __module__ = 'rtpgen.ipc_pack_pb2'
  # @@protoc_insertion_point(class_scope:RtpgenIPCmsgV1)
  ))
_sym_db.RegisterMessage(RtpgenIPCmsgV1)


# @@protoc_insertion_point(module_scope)
